...
import { useMount, useUpdateEffect, useOnceEffect } from '@reactuses/core'
import {
  Dices,
  MinusIcon,
  PlusIcon,
...
export function FormDialog({ currentRow }: FormDialogProps) {
  ...
  useUpdateEffect(() => {
    function loadValues() {
      ...
      form.reset({
        ...
        status: (currentRow?.status as unknown as string) || '1',
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      navigate({ to: '/sales/$id', params: { id: data.data.id } })
    },
    onSettled: () => {
  ...
  const generate = () => {
    form.setValue(
      'reference',
      'SO-' + faker.string.alphanumeric({ length: 8, casing: 'upper' }),
